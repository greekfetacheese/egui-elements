...
impl ThemeEditor {
        ...
        ui.vertical_centered(|ui| {
            ...
            CollapsingHeader::new("Theme Frames").show(ui, |ui| {
                CollapsingHeader::new("Window Frame").show(ui, |ui| {
                    self.frame_settings("window_frame", &mut theme.window_frame, &colors, ui);
                });
            ...
            CollapsingHeader::new("Theme Colors").show(ui, |ui| {
                ...
                self.hsla_edit_button
                    .show("widgetbg", ui, &mut theme.colors.widget_bg);

                ui.label("Hover");
                self.hsla_edit_button
                    .show("hover", ui, &mut theme.colors.hover);

                ui.label("Text");
                self.hsla_edit_button
                    .show("text1", ui, &mut theme.colors.text);

                ui.label("Text Muted");
                self.hsla_edit_button
                    .show("text_muted1", ui, &mut theme.colors.text_muted);

                ui.label("Highlight");
                self.hsla_edit_button
                    .show("highlight1", ui, &mut theme.colors.highlight);

                ui.label("Border");
                self.hsla_edit_button
                    .show("border1", ui, &mut theme.colors.border);

                ui.label("Accent");
                self.hsla_edit_button
                    .show("accent", ui, &mut theme.colors.accent);

                ui.label("Error");
                self.hsla_edit_button
                    .show("error1", ui, &mut theme.colors.error);

                ui.label("Warning");
                self.hsla_edit_button
                    .show("warning1", ui, &mut theme.colors.warning);

                ui.label("Success");
                self.hsla_edit_button
                    .show("success1", ui, &mut theme.colors.success);

                ui.label("Info");
                self.hsla_edit_button
                    .show("info1", ui, &mut theme.colors.info);

                theme.remap_derived_frames(&old_colors);
            });

            CollapsingHeader::new("Typography").show(ui, |ui| {
                ui.label("Very Small");
                ui.add(Slider::new(&mut theme.typography.very_small, 0.0..=100.0).text("Size"));

                ui.label("Small");
                ui.add(Slider::new(&mut theme.typography.small, 0.0..=100.0).text("Size"));

                ui.label("Normal");
                ui.add(Slider::new(&mut theme.typography.normal, 0.0..=100.0).text("Size"));

                ui.label("Large");
                ui.add(Slider::new(&mut theme.typography.large, 0.0..=100.0).text("Size"));

                ui.label("Very Large");
                ui.add(Slider::new(&mut theme.typography.very_large, 0.0..=100.0).text("Size"));
    ...
    fn tesellation_settings(&mut self, theme: &Theme, ui: &mut Ui) {
        ...
        ui.label(text);
        ui.add(
            DragValue::new(&mut options.bezier_tolerance)
                .speed(0.1)
                .range(0.0..=1.0),
        );
        ...
        ui.label(text);
        ui.add(
            DragValue::new(&mut options.epsilon)
                .speed(0.1)
                .range(0.0..=1.0),
        );
    ...
    fn button_visuals(&mut self, colors: ThemeColors, visuals: &mut ButtonVisuals, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            self.hsla_edit_button
                .show("border1", ui, &mut visuals.border.color);
        });
        ...
        ui.horizontal(|ui| {
            ...
            self.hsla_edit_button
                .show("border_hover1", ui, &mut visuals.border_hover.color);
        });
        ...
        ui.horizontal(|ui| {
            ...
            self.hsla_edit_button
                .show("border_click1", ui, &mut visuals.border_click.color);
        });
        ...
        ui.horizontal(|ui| {
            ...
            */
            ...
            color_edit_button_srgba(ui, &mut visuals.shadow.color, Alpha::BlendOrAdditive);
        });
    ...
    fn combo_box_visuals(
        ...
        ui.horizontal(|ui| {
            ...
            self.hsla_edit_button
                .show("border_open1", ui, &mut visuals.border_open.color);
        });
    ...
    fn widget_settings(&mut self, theme: &mut Theme, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            self.hsla_edit_button
                .show("bg_fill1", ui, &mut widget_visuals.bg_fill);
        });

        ui.label("Weak Background Fill Color");

        ui.horizontal(|ui| {
            let color = self.color_select("2", widget_visuals.weak_bg_fill, &theme.colors, ui);
            if let Some(color) = color {
                widget_visuals.weak_bg_fill = color.color32();
            }

            self.hsla_edit_button
                .show("weak_bg_fill1", ui, &mut widget_visuals.weak_bg_fill);
        });

        ui.label("Background Stroke Width");
        ui.add(Slider::new(&mut widget_visuals.bg_stroke.width, 0.0..=10.0));

        ui.label("Background Stroke Color");
        ui.horizontal(|ui| {
            let color = self.color_select("3", widget_visuals.bg_stroke.color, &theme.colors, ui);
            if let Some(color) = color {
                widget_visuals.bg_stroke.color = color.color32();
            }

            self.hsla_edit_button
                .show("bg_stroke_color1", ui, &mut widget_visuals.bg_stroke.color);
        });
        ...
        ui.add(Slider::new(&mut widget_visuals.fg_stroke.width, 0.0..=10.0));

        ui.label("Foreground Stroke Color");
        ui.horizontal(|ui| {
            let color = self.color_select("4", widget_visuals.fg_stroke.color, &theme.colors, ui);
            ...
            self.hsla_edit_button
                .show("fg_stroke_color1", ui, &mut widget_visuals.fg_stroke.color);
        });
    ...
    */
    ...
    fn frame_settings(&mut self, id: &str, frame: &mut Frame, colors: &ThemeColors, ui: &mut Ui) {
        ...
        ui.label("Fill Color");
        self.hsla_edit_button
            .show(&format!("{id}_fill"), ui, &mut frame.fill);

        ui.label("Stroke Width & Color");
        self.edit_stroke(id, colors, &mut frame.stroke, ui);
    }

    fn _select_widget_state(&mut self, ui: &mut Ui) {
        ComboBox::from_label("")
    ...
    fn edit_stroke(&mut self, id: &str, colors: &ThemeColors, stroke: &mut Stroke, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            let color = self.color_select(&format!("{id}_stroke_pal"), stroke.color, colors, ui);
            if let Some(color) = color {
            ...
            self.hsla_edit_button
                .show(&format!("{id}_stroke"), ui, &mut stroke.color);
        });
    }
}
    ...
    fn hsla_picker_ui(&mut self, ui: &mut Ui, hsla: &mut Hsla) -> bool {
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                ui.add_space(10.0);
                changed |= ui
                    .add(Slider::new(&mut hsla.h, 0.0..=360.0).text("Hue"))
                    .changed();
                changed |= ui
                    .add(Slider::new(&mut hsla.s, 0.0..=100.0).text("Saturation"))
                    .changed();
                changed |= ui
                    .add(Slider::new(&mut hsla.l, 0.0..=100.0).text("Lightness"))
                    .changed();
                changed |= ui
                    .add(Slider::new(&mut hsla.a, 0.0..=1.0).text("Alpha"))
                    .changed();
            });
...
fn hue_slider(ui: &mut Ui, hsla: &mut Hsla) -> bool {
    ...
    painter.line_segment([line_start, line_end], Stroke::new(2.0, Color32::WHITE));
