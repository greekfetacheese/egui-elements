...
impl<'a> SecureTextEdit<'a> {
    ...
    pub fn show(mut self, ui: &mut Ui) -> SecureTextEditOutput {
        self.visuals = self
            .visuals
            .or_else(|| Theme::text_edit_visuals_from_ctx(ui.ctx()));
        let frame = self.frame;
        let where_to_put_background = ui.painter().add(Shape::Noop);
        let fill = self
            .background_color
            .map(Fill::solid)
            .or_else(|| self.visuals.as_ref().map(|v| v.bg))
            .unwrap_or(Fill::solid(ui.visuals().extreme_bg_color));
        ...
        if frame {
            ...
            let shadow = text_edit_visuals
                .as_ref()
                .map(|v| v.shadow)
                .unwrap_or(egui::Shadow::NONE);

            let (fill, stroke) = if is_interactive {
                let border = if output.response.has_focus() || output.response.hovered() {
                    text_edit_visuals
                        .as_ref()
                        .map(|v| v.border_open)
                ...
                let border = text_edit_visuals
                    .as_ref()
                    .map(|v| v.border)
                    .unwrap_or(inactive.bg_stroke);
                (Fill::solid(Color32::TRANSPARENT), border)
            };
            ui.painter().set(
                where_to_put_background,
                crate::visuals::paint_shape(frame_rect, fill, stroke, corner, shadow),
            );
        }
        output
    }
    ...
    fn show_content(mut self, ui: &mut Ui) -> (SecureTextEditOutput, Option<TextEditVisuals>) {
        ...
        if self.interactive {
            if let Some(pointer_pos) = ui.ctx().pointer_interact_pos() {
                ...
                let cursor_at_pointer = galley
                    .cursor_from_pos(pointer_pos - text_draw_rect.min + singleline_offset_vec);
        ...
        if !owns_ime_events {
            ...
            if let Some(range) = state.cursor.char_range() {
                state
                    .cursor
                    .set_char_range(Some(CCursorRange::one(range.primary)));
            }
        }
        ...
        */
        response.widget_info(|| {
            WidgetInfo::text_edit(ui.is_enabled(), String::new(), String::new(), String::new())
        });
...
fn secure_text_edit_events(
    ...
    for event in events_filtered {
        ...
        let new_ccursor_range_opt: Option<CCursorRange> = match event {
            ...
            Event::Text(mut text_to_insert) => {
                if !text_to_insert.is_empty() && text_to_insert != "\n" && text_to_insert != "\r" {
                    ...
                    let mut final_text_to_insert =
                        if text_to_insert.chars().count() > space_available {
                            text_to_insert
                                .chars()
                                .take(space_available)
                                .collect::<String>()
                    ...
                    let mut current_ccursor = min;
                    let chars_inserted = text
                        .insert_text_at_char_idx(current_ccursor.index.0, &final_text_to_insert);
                    current_ccursor.index += chars_inserted;
            ...
            }) =>
            {
                if multiline {
                    ...
                    if space_available > 0 {
                        let mut current_ccursor = min;
                        let chars_inserted =
                            text.insert_text_at_char_idx(current_ccursor.index.0, "\n");
                        current_ccursor.index += chars_inserted;
                        text_mutated_this_event = true; // Mark mutation
                        Some(text::CCursorRange::one(current_ccursor))
                    } else {
            ...
            Event::Key {
                ...
                if text_mutated_this_event {
                    Some(text::CCursorRange::one(text::CCursor::new(new_cursor_idx)))
                } else {
                    None
                }
            }
        ...
        if text_mutated_this_event {
            ...
            let mut job = if multiline {
                ...
                LayoutJob::simple_singleline(display_text_for_layout, font_id.clone(), text_color)
            };
    ...
    (text_changed_in_total, cursor_range, current_galley)
}
...
mod tests {
    ...
    fn gnome_wayland_ime_types_forwards_not_in_reverse() {
        ...
        #[allow(deprecated)]
        run_pass(&ctx, &mut text, id, vec![Event::Ime(ImeEvent::Enabled)]);

        for ch in ['h', 'e', 'l', 'l', 'o'] {
            run_pass(&ctx, &mut text, id, gnome_wayland_keystroke(ch));
        }

        assert_eq!(text, "hello", "IME keystrokes must append, not insert at 0");
    }
    ...
    fn plain_text_events_type_forwards() {
        ...
        for ch in ['h', 'e', 'l', 'l', 'o'] {
            run_pass(&ctx, &mut text, id, vec![Event::Text(ch.to_string())]);
        }
    ...
    fn empty_ime_preedit_without_composition_does_not_reset_cursor() {
        ...
        run_pass(&ctx, &mut text, id, vec![Event::Text("ab".to_owned())]);
        run_pass(&ctx, &mut text, id, vec![ime_preedit("")]);
        run_pass(&ctx, &mut text, id, vec![Event::Text("c".to_owned())]);
