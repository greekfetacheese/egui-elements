use egui::{
    Atom, AtomExt as _, AtomKind, AtomLayout, AtomLayoutResponse, Color32, Image, IntoAtoms,
...
impl<'a> Button<'a> {
    ...
    /// Set the background fill of the button (idle). Hover / click still use visuals.
    #[inline]
    pub fn bg_fill(mut self, fill: impl Into<Fill>) -> Self {
        self.bg_fill = Some(fill.into());
        self
    }

    /// Set a solid background color of the button (idle).
    #[inline]
    pub fn bg_color(self, color: Color32) -> Self {
        self.bg_fill(Fill::solid(color))
    }
    ...
    pub fn atom_ui(self, ui: &mut Ui) -> AtomLayoutResponse {
        ...
        let frame = egui::Frame::new().inner_margin(button_padding);
        ...
        let response = if ui.is_rect_visible(prepared.response.rect) {
            ...
            let (fill, stroke, corner_radius) = if let Some(v) = &visuals {
                if is_active {
                    ...
                    let bg = bg_fill.unwrap_or(v.bg);
                    (bg, v.border, v.corner_radius)
                }
            } else {
                (
                    Fill::solid(interact_visuals.weak_bg_fill),
            ...
            let fill = match selected {
                ...
                true => visuals
                    .as_ref()
                    .map(|v| v.bg_selected)
                    .unwrap_or(Fill::solid(interact_visuals.weak_bg_fill)),
            };

            let text_color = visuals
                .as_ref()
                .map(|v| v.text)
                .unwrap_or(interact_visuals.text_color());
            ...
            if visible_frame {
                ...
                let expansion = interact_visuals.expansion;
                let widget_rect = prepared.response.rect.expand(expansion);
                ui.painter().add(crate::visuals::paint_shape(
                    widget_rect,
                    fill,
                    stroke,
                    corner_radius,
