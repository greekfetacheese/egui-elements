...
use egui::{
    Align, Color32, FontSelection, Image, Pos2, Rect, Response, Sense, Stroke, TextWrapMode, Ui,
    Vec2, Widget, WidgetText, epaint::TextShape, style::WidgetVisuals, text::LayoutJob,
};
use std::sync::Arc;
        ...
        if ui.is_rect_visible(rect) {
            ...
            let fill = if self.selected {
                label_visuals
                    .as_ref()
                    .map(|v| v.bg_selected)
                    .unwrap_or(Fill::solid(visuals.bg_fill))
            } else if response.hovered() || response.has_focus() {
                match self.interactive {
                    true => label_visuals
                        .as_ref()
                        .map(|v| v.bg_hover)
                        .unwrap_or(Fill::solid(visuals.weak_bg_fill)),
                    false => Fill::solid(Color32::TRANSPARENT),
                }
            } else {
                label_visuals
                    .as_ref()
                    .map(|v| v.bg)
                    .unwrap_or(Fill::solid(Color32::TRANSPARENT))
            };

            let stroke = if self.selected {
                label_visuals
                    .as_ref()
                    .map(|v| v.border_hover)
            ...
            let corner = label_visuals
                .as_ref()
                .map(|v| v.corner_radius)
                .unwrap_or(visuals.corner_radius);
            let background_rect = rect.expand(expansion);

            // Labels never inherited button shadow — idle chrome is transparent
            // fill + transparent stroke. Painting `button_visuals.shadow` here
            // made every label look filled at rest and muddied highlight.
