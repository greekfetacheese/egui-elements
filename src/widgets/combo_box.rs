...
use egui::{
    ...
    Response, ScrollArea, Sense, Stroke, TextWrapMode, Ui, Vec2, WidgetText, epaint::Shape,
    style::WidgetVisuals,
};
...
impl ComboBox {
    ...
    pub fn show_ui<R>(
        ...
        let visuals = self
            .visuals
            .or_else(|| Theme::combo_box_visuals_from_ctx(ui.ctx()));
        ...
        let inner =
            popup.show(|ui| {
                ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        ui.set_width(ui.available_width().max(
                            button_response.rect.width() - ui.spacing().button_padding.x * 2.0,
                        ));
...
fn combo_box_with_image_button(
    ...
    if ui.is_rect_visible(rect) {
        ...
        if let Some(vis) = combo_box_visuals {
            vis.paint_at(ui.painter(), background_rect, fill, stroke);
        } else {
        ...
        let label_rect_width =
            (icon_rect.left() - content_total_rect.left() - icon_spacing).max(0.0);
        ...
        if let Some(icon_painter) = icon_painter {
            ...
            paint_default_icon(ui.painter(), icon_rect, combo_box_visuals, &visuals);
        }
    }
