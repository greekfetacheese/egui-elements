...
impl DemoApp {
                ...
                ui.horizontal(|ui| {
                    ...
                    for (name, color) in colors {
                        ui.vertical(|ui| {
                            ...
                            let (rect, _) =
                                ui.allocate_exact_size(vec2(96.0, 44.0), Sense::hover());
                            ui.painter().rect(
                                rect,
        ...
        frame2.show(ui, |ui| {
            ui.vertical(|ui| {
                ...
                let nested: [(&str, &str, Color32); 2] = [
                    ("GreekFetaCheese", "$1,600.00", self.theme.colors.success),
                    ("Cold storage", "$12,400.00", self.theme.colors.text),
                ];
    ...
    fn widget_gallery(&mut self, id_salt: &str, ui: &mut Ui) {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            let btn = Button::new(RichText::new("Button").size(size).color(text_color))
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            ui.add(
                Label::new(
                    RichText::new("Label (interactive)")
                        .size(size)
                        .color(text_color),
                    None,
                )
                ...
                frame1.show(ui, |ui| {
                    ...
                    ui.vertical_centered(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            ui.spacing_mut().item_spacing.x = 16.0;
                            if ui
                                .checkbox(&mut self.confirm_password, "Confirm password")
                                .changed()
                            {
                                self.credentials.set_confirm_password(self.confirm_password);
                            }
                            if ui
                                .checkbox(&mut self.virtual_keyboard, "Virtual keyboard")
                                .changed()
                            {
                ...
                frame1.show(ui, |ui| {
                    ...
                    ui.vertical_centered(|ui| {
                        ...
                        if self.qr.has_error() {
                            ui.label(
                                RichText::new(
                                    self.qr.error().map(|e| e.to_string()).unwrap_or_default(),
                                )
